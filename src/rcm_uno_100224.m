...
%% Parameters, user edit:
    ...
    Objective_mag = 40;		% objective magnification (compute FOV)
    ...
    PreView     = 0;		% 1= preview loop, 0= high-res data capture
    HistEq      = 0;        % 1 = histogram eq before display; 0= display raw image
    PreViewLinearize = 1;   % linearize scan coordinates before preview display
    ...
    OutAmpX_V = 0.40;		% output amplitude, cosine wave voltage peak
    OutAmpY_V = 0.40;		% output amplitude, ramp voltage peak
...
%% Run the DAQ/Image loop
	...
	if ( PreView )
        ...
        if( HistEq==1 )
            rasterIuDisp = histeq(rasterIu);
            imshow( rasterIuDisp, ...
	            XData=[0, fovX_um], YData=[0, fovY_um] );
        else
            rasterIuDisp = rasterIu;
            imshow( rasterIuDisp, DisplayRange=[sigMin_V, sigMax_V], ...
	            XData=[0, fovX_um], YData=[0, fovY_um] );
        end
